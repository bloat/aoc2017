...
(defn diff [row]
  (let [[smallest biggest]
        (reduce (fn [[small big] x] (vector (if small (min small x) x)
                                            (if big (max big x) x)))
                [nil nil]
                row)]
    (- biggest smallest)))
...
(defn evdiv [row]
  (let [[x y]
        (first (filter (fn [[x y]] (not (= x y))) (filter (fn [[x y]] (zero? (rem x y))) (for [x row y row] [x y]))))]
    (/ x y)))
